...
def calc(i,tmin,step,theta,theta_s,arr1,arr2,X,Y,ax,frame):
    plt.cla()
    if i == frame - 1:
        sys.exit()
    t = (tmin + (step * i)) * 10 ** -12
...
def main(tmin,tmax,step,abs,frame):
    ...
    anim = animation.FuncAnimation(fig,calc,fargs=(tmin,step,45,45,arr1,arr2,X,Y,ax,frame),interval = 50,frames=frame)
    plt.show()
